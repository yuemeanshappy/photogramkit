...
def extract_time_for_photos_in_one_camera(date_folder, camera):
	...
	Outputs:
		timestamps: a list of tuple where in each tuple there are two elements (photo name and timestamp)
	"""
...
def group_photos_by_timestamp(timestamps, max_time_diff=timedelta(minutes=3)):
	...
	Outputs:
		groups: a list of lists where each list is a group of images that have time differences smaller than max_time_diff. 
				For example:
